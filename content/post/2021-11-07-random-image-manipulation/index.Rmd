...
This is a process that runs daily and applies a **random number of transformations** in a **random order** with **random settings** to a **randomly selected image** from my [SmugMug site](https://twilson.smugmug.com/). You can see a [list of the results](/image-randomize/) that are currently available. I started to try to do the math as to the number of possible outcomes and...it's beyond me. The formula looked something like:

$$
[Random] \times [Random] \times [Random] \times ... \times [Random]
$$
$$
[Random]^{[Something]}
$$
...
\approx\infty
$$
Obviously, this is not real math. But, tracking down the _actual_ number of possible variations (there are some of the randomizations that set a value between 0 and 1...out to 15 decimal places, which, I think, means a quadrillion variations for that setting alone; practically speaking, it's probably more like 100 or 1,000 different values that the naked eye would notice, but you get the idea how my head started to hurt and I just moved on).
...
This project qualifies as generative art (or generative "aRt," specifically, but if you're interested in that sort of thing, check out [Danielle Navarro's amazing work](https://art.djnavarro.net/) or [Claus O. Wilke's work](https://clauswilke.com/art/)—including his [NFTs](https://objkt.com/profile/clauswilke/created?search=&sortBy=askAsc)!—or [Koen Derks's aRtsy package that is specifically ggplot-oriented](https://koenderks.github.io/aRtsy/)). 
...
I think it's actually reasonably elegant design-wise (if I do say so myself). All that is required for me to add another transformation is to create a new file for that transformation and add the path to that file to the original tibble.
...
* I could just make a [dedicated folder under the 'content' folder](https://github.com/gilliganondata/gilliganondata_site/tree/main/content/image-randomize) and put all of the scripts and details there. This is a sibling folder of the default `post` folder, which is where straight-up posts (like the one your reading now get built)

* I could create an R script that would actually build the .Rmd file that would display the before and after images and the transformations applied. That script is [here](https://github.com/gilliganondata/gilliganondata_site/blob/main/content/image-randomize/build_rmd.R), but [this short video](https://www.youtube.com/watch?v=nBIzgdUBDOk) from CradleToGraveR was really helpful in getting the details right on that.

* When building that .Rmd file, I included a couple of handy things in the YAML header for when I built my list template (see the next item): a `type` parameter that simply populated with "image-randomization," and then `img_original` and `img_transformed` parameter that contained the relative paths to the original and output images.

* I had to read/watch the Hugo documentation on [list templates](https://gohugo.io/templates/lists/) a few times to fully grok them, but this allowed me to make the [page(s) of thumbnails](https://gilliganondata.netlify.app/image-randomize/) showing before and after images. The source for that is in the [`/layouts/image-randomize`](https://github.com/gilliganondata/gilliganondata_site/blob/main/layouts/image-randomize/image-randomize.html.html) folder, and the path and the filename for that are key. There is logic in that template that says to "just use the files that are of "type" (from the YAML header) of "image-randomization" and then use the `img_original` and `img_transformed` values to actually create the thumbnails. This...is pretty cool! (There is some inefficiency with the images themselves, as it currently just changes the dimensions of the images, but is loading the full images, so the page weight is heavier than it needs to be; that's a fix for another day)
...
I _could_ simply have a process whereby a daily tweet has a broken link for less than a minute. While I've certainly cut a few corners here and there on this effort, that's a small bridge too far.

So, instead:

* A couple of lines of code at the very end of the [script that builds the `.Rmd` file for the web page](https://github.com/gilliganondata/gilliganondata_site/blob/main/content/image-randomize/build_rmd.R) create a list with three items: the ultimate URL for the new page, the path to the original image, and the path to the transformed image. The script then writes that to a file using `saveRDS`.
...
A few examples that are at least in the general direction of interesting:
