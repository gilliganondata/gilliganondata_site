...
_Causation_ is where marketers (and analysts) tend to make dangerous leaps between what a given data set can and cannot do. When it comes to _making decisions_, arguably, _causation is key_. We want to know, _given a set of options when it comes to what we are going to **do**, which one will have the greatest positive **effect**?_ (See how "effect" worked it's way in there? You know the phrase: "cause and effect." Boom! There's "cause!")
...
[^3]: I absolutely _loathe_ the phrase "actionable insights," as it gets tossed around with a frequency and abandon that rivals the consumption of frankfurters on Dollar Dog Night at the ballpark. I'm coming to believe that my reaction to the phrase is because I've actually had the correct intuition as to what different reports and analyses could (and could _not_) reasonably deliver. The phrase implies that there is _actionability_ (a decision can be made) based on an _insight_...which means there is an underlying _causal finding_.

There _is_ a use of the data that does not have a reliance on causation. That use is simply _measuring performance_: how much traffic came to the website last month? How many leads have converted to opportunities this quarter? How many customers churned last year?

With a "causation first" focus, we can boil the subject of this post down to Gilligan's Simplified Causal Evidence Ladder^TM^:

![](images/evidence-ladder.png)
THAT was the TL;DR! (I told you I sucked at that.) Let's dive into each level.
...
* To what extent are traffic to the website and revenue from the website correlated with each other?
...
The terminology starts to add some syllables when we move to Level 2 and Level 3, so let's step off the ladder for am minute to hit one of those key terms: **counterfactual**.

The basic idea of a counterfactual is that, if we had our 'druthers, and if we weren't limited by the current constraints of our ability to manipulate the space-time continuum, any time we had an important decision to make with multiple choices, we would make... _all of them_. We would split the universe, try a different option in each universe, see which one works out best, and then toss out the other universes and proceed with the choice (the decision!) that yielded the best results.
...
* Decrease your monthly spend by 20% (and increase your Paid Social spend)
...
This level is, arguably, the messiest. But, it's also the most alluring, as it's where the idea that, given a sufficient volume of data fed into the right model, "the truth" will emerge. This is... not true.

But, inferential statistics[^4] is all about taking a big pile of data that has already been generated and drawing conclusions (inferences) about the way the world works. The techniques for doing this are so broad and so deep that the posts linked to at the beginning of this post split this level into multiple levels!

[^4]: Is "inferential statistics" _the right term_ for this? I'm not entirely sure (but I'm getting increasingly comfortable with uncertainty). It's the broadest term I've found for what I've described in this section.

When we start to look at historically generated data through a counterfactual lens, we start to see that data in two parts:
...
We have to try to use statistical modeling to try to make an informed guess as to the relationship between our Google Adwords spend, our Paid Social spend, and our overall results (revenue). Pretty quickly, we'll likely decide we need to look at more data than just last month's&#8212;can we get a year or two's worth, maybe? Then, we set about trying to tease apart and account for as many things as we can think of (this is why "domain expertise" is so important for both the analyst and the data scientist).
...
What quasi-experimentation is _not_ is just a half-assed experiment: "Yeah... I know I should've randomly assigned which DMAs received the media and which ones did not when I did my matched market test, but I just listed them in numerical order and used the first half of the list as my treatment DMAs and the second half of the list as my control."
