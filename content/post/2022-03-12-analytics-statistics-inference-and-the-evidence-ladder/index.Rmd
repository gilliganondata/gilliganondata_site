...
* A **statistic** is descriptive and (reasonably) deterministic
* **Inferential statistics** are about making predictions about a population from a sample of observations from that population and about describing relationships between different characteristics within a data set
* **Causal inference** is about describing the impact that one (or multiple) factors have (or will have) on something else

_Causation_ is where marketers (and analysts) tend to make dangerous leaps between what a given data set can and cannot do. When it comes to _making decisions_, arguably, _causation is key_. We want to know, _given a set of options when it comes to what we are going to **do**, which one will have the greatest positive **effect**?_

This doesn't mean that the _only_ reason we use data is to find causation and make decisions, but it's one of the main expectations we tend to have of the data.

I absolutely _loathe_ the phrase "actionable insights," as it gets tossed around with a frequency and abandon that rivals the consumption of frankfurters on Dollar Dog Night at the ballpark. I'm coming to believe that my reaction to the phrase is because I've actually had the correct intuition as to what different reports and analyses could (and could _not_) reasonably deliver. The phrase implies that there is _actionability_ (a decision can be made) based on an _insight_...which means there is an underlying _causal finding_.
...
* The maximum daily revenue from the site
* The minimum daily revenue from the site
* To what extent are traffic to the website and revenue from the website correlated with each other?

The list goes on and on and on. And, putting aside the fact that website analytics data is inherently messy to collect, these "statistics" are all simply _facts_ about the website's performance last month.
...
The terminology starts to add some syllables at this next level. And, this level is, arguably, the messiest. It's actually so broad and so deep that the posts linked to above split this level into multiple levels!
